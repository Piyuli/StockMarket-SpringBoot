...
private Logger logger = LoggerFactory.getLogger(this.getClass());
	...
	public void run(String... args) throws Exception {
		...
		Sectors sector = new Sectors(" Sector2", "It takes care of financial problems");
		sectorRespository.save(sector);
		
		StockExchange stockExchange = new StockExchange("BSE", "Primary", "Nehru Road", "Crucial");
		stockExchangeRepository.save(stockExchange);
		
		StockExchange stockExchange1 = new StockExchange("NSE", "Primary", "White Road", "Highly Critical");
		stockExchangeRepository.save(stockExchange1);
		
		StockPrice stockPrice = new StockPrice((float)40.15, "10-9-14", "5:17 AM");
		...
		Company company = new Company("company1", (float)200000000, "Piyuli", "Saurav, Meghna, Arpita", "It is working well.", ipo, Arrays.asList(stockExchange), Arrays.asList(stockPrice), sector);
		companyRepository.save(company);
		
		Company company1 = new Company("company2", (float)200000000, "Aman", "Ritwik, Vineet, Pranesh", "It is working fine.");
		...
		CompanyCode companyCode = new CompanyCode(2121);
